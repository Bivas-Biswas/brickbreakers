...
for (let x = groundX + 35; x <= play_width + 100 ; x += 75) {
    for (let y = groundY + 10; y < height/2 - 100; y += 40) {
        if(count === 1 || count === 2 || count === 5 ||count === 6|| count === 7 ||count === 12 ||count === 25||count === 30||count === 31||count === 32||count === 35||count === 36){
...
Events.on(engine, 'collisionEnd', function (event) {
    ...
    if(engine.world.bodies.length === 6){
        ...
        engine.events = {}
        btn.addEventListener('click', function (){
            window.open('../level03/level03.html', '_self')
        })
    }
});
